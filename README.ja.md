...
**1台のマシンに複数セッションを立てる人。** マウントと認証済み接続を1つずつ共有し、`afws-lock`が2つのセッションを同じGPUから遠ざけ、Claudeセッション同士は互いが見つけたことを尋ね合えます（再調査が不要）。

副次的に得られるもの。コマンドごとではなくホストごとに1回のパスワード入力、書くときは手元のエディタと道具、そして最後のセッションが抜けた時点で自分を解放するマウントと接続。
