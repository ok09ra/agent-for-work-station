...
事前の宣言は要りません。両側ともMac上のただのパスであり、シェルコマンドはワークスペースに限定されず、ファイルツールもワークスペース外の絶対パスを受け付けます。`AFWS_ADD_DIR`が必要なのは2つの場合だけです。エージェントがローカルのディレクトリに**書き込む**必要がある場合（`codexfws`では唯一の手段。サンドボックスが起動時に決まるため）と、ローカルディレクトリにあるスキルやコマンドを読み込ませる場合です。
