...
## Claude Code asks whether to trust the folder
...
The detached sshfs process writes to `~/.claudefws/logs/SESSION.sshfs.log`, and
...
Scripts piped to `claudefws-run` run under remote `bash -s`. For code written for another shell, invoke that shell explicitly:
...
## Claude tries to run tests locally

Confirm that the session was started through `claudefws`. A normal `claude` launch does not receive the remote-execution instructions. Exit it and restart through `claudefws` with the target project selected.

## A remote command hangs instead of running

Every remote command goes through the shared SSH connection at
`~/.claudefws/control/HOST.sock`. If that connection is gone, ssh falls back to
...
## `claudefws-peers` shows nothing, or a session is missing

The registry only holds sessions started through `claudefws`. A session started with plain `claude` is not registered, even in a mounted workspace.
...
## `claudefws-peers` reports the status as `unknown`

Status comes from `claude agents --json`. Run it directly to see why it failed; the usual cause is that Claude Code is not signed in:
...
A lock past its TTL is reported as `stale`. Confirm with `claudefws-peers` and `claude agents` that the holder is really gone, then release it explicitly:
...
`claudefws-lock` on a shared build or output directory prevents the same
