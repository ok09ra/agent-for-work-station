...
共有されたマウントは、最後に抜けたセッションが解放します。同じマウント内で他のセッションがまだ作業している状態で終了したセッションは、マウントを残し、そのことを表示します。バックグラウンドセッションは後片付けを行うlauncherプロセスが残らないため、マウントをそのまま残します。それらは`afws-umount --orphaned`で解放できます。
