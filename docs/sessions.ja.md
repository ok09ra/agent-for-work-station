...
共有されたマウントは、最後に抜けたセッションが解放します。同じマウント内で他のセッションがまだ作業している状態で終了したセッションは、マウントを残し、そのことを表示します。各対話型launcherは独立したwatchdogも起動し、launcherが強制終了・クラッシュした場合も同じ「最後の利用者か」の確認と後片付けを行います。ClaudeまたはCodexのプロセスが生き残っていれば、その終了まで待ってから解放します。バックグラウンドセッションはマウントをそのまま残し、`afws-umount --orphaned`で解放できます。
