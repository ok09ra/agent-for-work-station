...
プロジェクトはリモートにありますが、**作業の材料**はローカルにあることが多いはずです。論文、ノート、手元の試し計算。多くの場合、事前の準備は要りません。両側ともMac上のただのパスであり、シェルコマンドはワークスペースに限定されず、Claude Codeはファイルツール用に`/add-dir`をセッション中に受け付けます。会話の中でディレクトリ名を言えば足ります。

`AFWS_ADD_DIR`は、それでは足りない場合のためのものです。毎回同じディレクトリを使う場合や、エージェントがローカル側にも**書き込む**必要がある場合です。`:`区切りで指定します。
