...
プロジェクトはリモートにありますが、**作業の材料**はローカルにあることが多いはずです。論文、ノート、手元の試し計算。`AFWS_ADD_DIR`にそれらを`:`区切りで指定すると、1つのセッションがそれらを読み、結論をリモートのプロジェクトへ書けます。
...
これがないと、ローカルでの読み込みとリモートでの作業が別セッション・別履歴に分かれ、その間の受け渡しが手作業になります。

`codexfws`ではこれは不要です。Codexはワークスペース外の読み取りを許可するサンドボックス経由で読むため、ローカルパスは指定なしで読めます。変数が設定されていればそう表示します。Codexが制限するのは書き込みのほうです。
